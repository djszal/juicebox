...
async function updateUser(id, fields = {}) {
  const setString = Object.keys(fields)
    ...
    .join(", ");
  // console.log("yyyyyyyyyyyyy", setString);
  if (setString.length === 0) {
    return;
  }
  const test = Object.keys(fields);
  // console.log("<<<<<<<<<<<<", test);
  ...
  try {
    ...
    RETURNING *;
    `,
      Object.values(fields) //=== [NewName Sogood, Lesterville, KY]
    );
    const test2 = Object.values(fields);
    // console.log("@@@@@@@@@@@@@@@@", test2);
    return user;
  } catch (error) {
    throw error;
...
async function addTagsToPost(postId, tagList) {
  ...
  try {
    ...
    const test = await getPostById(postId);
    // console.log("0000000000", test);
    return test;
  } catch (error) {
    throw error;
  }
